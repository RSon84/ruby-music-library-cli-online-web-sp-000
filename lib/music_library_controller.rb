...
# =end
  ...
  def list_artists
    ...
    songs_sorted_by_artist = Artist.all.uniq.sort_by do |artist| #returns an array of artists sorted by artist name
      artist.name
    end
  ...
  if (1..Song.all.length).include?(input)
    array = Song.all.sort{|a, b| a.name <=> b.name}
      ...
      song = array[input+2]
     puts "Playing #{song.name} by #{song.artist.name}" if song.first
end
end
end
